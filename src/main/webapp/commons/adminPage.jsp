	...
	<c:when test="${not empty param.pageLink && totalPages > 1 }">
		<c:choose>
			<c:when test="${page.pageNo>1 && not empty param.pageLink }">
				<a class="cms-page-forward"  href="#none" lang="${param.pageLink}&pageNo=${((page.pageNo)-1)>0?(page.pageNo)-1:(page.pageNo)}">上一页</a>
			</c:when>
		</c:choose>
		<c:choose>
			<c:when test="${page.pageNo eq 1}">
				<a class="cms-page-forward"  href="#none" lang="${param.pageLink}&pageNo=1">1</a>
<!-- 				&nbsp; -->
			</c:when>						  
			<c:otherwise>
				<a class="cms-page-forward"  href="#none" lang="${param.pageLink }&pageNo=1">1</a>
						...
						<c:forEach var="i" begin="2" end="${page.pageNo}">
							<c:choose>
								<c:when test="${page.pageNo eq i }">
									<a class="cms-page-forward"  href="#none" lang="${param.pageLink }&pageNo=${i}">${i}</a>
<!-- 									&nbsp; -->
								</c:when>
								<c:otherwise>
									<a class="cms-page-forward"  href="#none" lang="${param.pageLink }&pageNo=${i }">${i}</a>
...
<!-- 						 ...&nbsp;   -->
						 <a class="cms-page-forward"  href="#none" lang="${param.pageLink }&pageNo=${totalPages }">${totalPages}</a>
<!-- 						 &nbsp;   -->
		        	</c:otherwise>
				</c:choose>
				<c:choose>
					<c:when test="${page.pageNo < totalPages}">
						<a class="cms-page-forward"  href="#none" lang="${param.pageLink }&pageNo=${(page.nextPage) < totalPages?page.nextPage: totalPages}">下一页</a>
					</c:when>
				</c:choose>
			</c:when>
		</c:choose>
